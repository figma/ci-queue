    ...
    module Redis
      ...
      class Worker < Base
        ...
        def requeue(test, offset: Redis.requeue_offset, skip_reservation_check: false)
          ...
          requeued = config.max_requeues > 0 && global_max_requeues > 0 && !config.known_flaky?(test_key) && eval_script(
            ...
            keys: [
              ...
              key('worker', worker_id, 'queue'),  # Worker-specific, not generation-scoped
        ...
        def acquire_master_role?
          ...
          # Generate unique ID for this population attempt
          @generation = SecureRandom.uuid

          # Use SET with NX and EX - embeds generation in lock value
          # Lock value format: "setup:#{generation}"
          @master = redis.set(
            key('master-status'),
            "setup:#{@generation}",
            nx: true,
          ...
          if @master
            ...
            @generation = nil  # Clear generation if we didn't win
          end
          ...
          chunks.each_slice(batch_size) do |chunk_batch|
            redis.multi do |transaction|
              chunk_batch.each do |chunk|
                # Store chunk metadata with TTL (generation-namespaced)
                transaction.set(
                ...
                # Track all chunks for cleanup (generation-namespaced)
                transaction.sadd(generation_key('chunks'), chunk.id)

                # Store dynamic timeout for this chunk (generation-namespaced)
        ...
        def resolve_chunk(chunk_id)
          # Fetch chunk metadata from Redis (generation-namespaced)
